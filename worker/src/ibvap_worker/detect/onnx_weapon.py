...
class OnnxWeaponDetector:
    ...
    def __init__(self, cfg: DetectorConfig, min_conf: float = 0.55, nms_iou: float = 0.45) -> None:
        ...
        # One instance serves every camera's stage thread. Without this, two
        # cameras checking at once both write self._buffer and one of them
        # runs the model on the other's crop -- a weapon "found" on the wrong
        # camera, boxed onto the wrong person.
