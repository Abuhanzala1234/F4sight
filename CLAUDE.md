...
**Project:** IBVAP — AI-Based Intelligent Video Analytics Platform for border surveillance on existing CCTV infrastructure
**Spec:** `docs/BUILD_SPEC.md` — read it before writing code. It is authoritative.
**Context:** Smart India Hackathon 2026, PS 26187, Theme: Blockchain & Cybersecurity, Team SW-73 (ByteForge). Hackathon timeline: optimise for a working, demoable system over completeness.
...
- Every module in `worker/src/ibvap_worker/` needs unit tests before it is considered
  done.
...
3. **Model cold start.** First inference takes ~2 s (TensorRT engine build or ORT graph
   optimisation). Warm up with dummy inferences at every batch shape at startup or the
   demo looks frozen.
4. **Browser cannot play RTSP.** Video goes through MediaMTX → HLS/WebRTC. Test this
   early; it surprises people on demo day. B-frames in the source break the browser's
   WebRTC player specifically — re-encode fixtures with `-tune zerolatency -bf 0`.
...
8. **GPU power capping on battery.** A laptop running on battery throttles the GPU
   (`nvidia-smi -q -d PERFORMANCE` shows `SW Power Cap: Active`) and per-frame inference
   goes from ~45 ms to 700+ ms. Stay plugged in for a live demo.
...
| A weapon alert must never be droppable by the spam rate ceiling | An armed-person alert is exactly the one alert a debounce ceiling must never eat (see `rate_limit_exempt_min_weight` in `config/rules.yaml`). |
...
For a long-running or unattended worker, use `scripts/worker_supervised.sh` instead of
`make worker` directly — it restarts on the rare uncatchable CUDA abort and loads `.env`.
